# -*- coding: utf-8 -*-
"""
C-III task T-S0 (increment 186): measure Lemma S's object BEFORE any
...
def main():
    ...
    worst = 0.0
    for i in range(240):
        # choose c ~ q = a*k in [2e3, 3e5]; u' in [1, 400]
        c = float(rng.integers(2_000, 300_000))
        up = float(rng.integers(1, 400))
        osc = (W1 - W0) * np.sqrt(up) / (c * np.sqrt(N))
        if osc < 3:
            continue
