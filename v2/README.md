...
골드바흐 이진 추측이 남긴 스칼라 하나를 다룬다:

```
C(N) = Σ_{n<N} Λ(n) μ(N-n) = o(N)
```

**할 일은 하나다 — 이 합에서 `log N`의 거듭제곱만큼의 상쇄를 증명 가능한
방식으로 끌어내는 것.** 크기는 이미 그보다 `N`의 거듭제곱만큼 아래에 있다.
막힌 것은 크기가 아니라 증명이고, 그래서 여기서 하는 일은 측정이 아니라
**어떤 메커니즘이 그 상쇄를 만드는지 찾는 것**이다.

무엇을 시도했고 어디서 막혔는지는 `paper/`, 지금 열려 있는 것은 `OPEN.md`.
