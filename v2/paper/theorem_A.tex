...
plainly what is \emph{standard}. The ingredients of Theorem~\ref{thm:A}
...
corrections it turns out to require. Theorem~\ref{thm:D} is, in genre, a
...
unsurprising is right to. Section~\ref{sec:lit} places both against the
record, including Lichtman's level of distribution beyond the
square-root barrier \cite{Li23}, which postdates \cite{HL}.
\end{abstract}
...
Section~\ref{sec:mechanism}: after divisor switching one \emph{completes}
the divisor sum, and on the complementary range $k\ge K$ the M\"obius
factor on the long variable squares itself away while the surviving
M\"obius sits on a variable of size $\le N^{1/2-\delta}$. The completion
...
\S\ref{sec:proof} carry out the argument in that order.
...
\begin{remark}[relation to Bombieri's asymptotic sieve]
\label{rem:bombieri}
The genre of Theorem~\ref{thm:D} is classical. Bombieri's asymptotic
sieve \cite{Bom76} shows that sieve weights alone cannot detect primes,
...
candidate consumptions, and Theorem~\ref{thm:C} then looks like an
...
\paragraph{Theorem~\ref{thm:A}.} The mechanism is standard. After the
...
(Bombieri--Vinogradov; the Goldston--Y\i ld\i r\i m estimate
\cite{GY} as used in \cite{HL}; the elementary density identity of
Proposition~\ref{prop:MT}) is off the shelf. The statement is offered as
a \emph{lemma about the Huang--Li reduction}, not as a contribution to
...
\paragraph{Theorem~\ref{thm:D} and Proposition~\ref{prop:E}.} See
Remark~\ref{rem:bombieri} for the relation to \cite{Bom76}.
Proposition~\ref{prop:E} is an observation --- Parseval bounds any
...
\paragraph{Beyond the square-root barrier.} Since \cite{HL} appeared,
...
The same line of work \cite{Li20,Li23} establishes a level of
distribution $3/5$ for the M\"obius function with triply
well-factorable weights; we have not verified the precise statement or
its location against the published versions, and cite it here as the
object the question below refers to rather than as a result we have
checked. Since
\cite{HL} require $EH_\mu(N^{\theta'})$ for a single $\theta'>1/2$, and
since Theorem~\ref{thm:C} identifies $w_k=\log k$ as the weight carrying
...
Two cautions attach to it. First, $EH_\mu$ as stated in \eqref{eq:EHmu}
...
be made against what the argument \emph{consumes}, which is a signed
sum, and not against the absolute-value form as written. Second,
Theorem~\ref{thm:D} closes extraction \emph{by divisor switching}; the
technology of \cite{Li23} is the Deshouillers--Iwaniec spectral large
sieve, a different mechanism, and Theorem~\ref{thm:D} says nothing about
it.
...
\bibitem{Li20} Jared Duker Lichtman, \emph{Primes in arithmetic
