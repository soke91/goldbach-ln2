...
\author{[author]}
\date{Working paper, increment 207 of the \texttt{goldbach-ln2}
program\\
...
\]
and Conjecture~\ref{conj:L} was stated for the $\mu$-families
$D(k)$ and $C_{k,k'}$, not for $C(N)$. Extending it to $C(N)$ took five
attempts, four of which this program recorded and withdrew; what
follows is the current form and the four facts now known about it. The
superseded versions and the reason each failed are in the repository's
correction record and are not repeated here.
...
\]
Then $V(N) = W(N)\,\mathfrak A(N)\,(1+o(1))$, and consequently
$V(N) \sim \mathfrak A(N)\,N\log N$.
\end{proposition}
...
\paragraph{The local factor is $\mathfrak A$, not $\mathfrak S$.}
This is the substitution that cost this program four withdrawn
statements. The singular series $\mathfrak S(N)$ is Hardy--Littlewood's
and is correct for the Goldbach \emph{count}; it was carried across to
the \emph{variance} of $C(N)$, where the right object is
$\mathfrak A(N)$. Both are products over $q\mid N$, which is why the
substitution survived. Rescaling each candidate to the measured mean,
so that only its shape in $N$ is judged, the residual standard
deviation over every even $N\le 1.6\cdot10^7$ is $0.000323$ for
$\mathfrak A$ against $0.245235$ for $\mathfrak S$ --- a factor of
$760$. Dividing through by $W(N)$ removes the analytic factor exactly,
so no asymptotic for $\sum(\log p)^2$ is assumed: the ratio
$V(N)/W(N)$ against $\mathfrak A(N)$ has mean $1.000000$ with standard
deviation $0.000145$ in the top octave, and agrees to five decimals in
each of the six radical cells from $2\mid N$ through
$2\cdot3\cdot5\cdot7\cdot11\cdot13\mid N$.

\subsection{The law, and what is measured about it}
...
\item \textbf{The bulk is Gaussian, under this scale and no other.}
  ...
  alone. Under an $\mathfrak S N$-based scale the same data give
  excess kurtosis $+0.1704$ at $z=98$: a normaliser that manufactures a
  heavy tail.

\item \textbf{The mask is real at every scale measured, and its law is
  not known.} Against an exactly computed floor---for a coin every
  second moment of every cell mean is closed form, $\mathrm{Var}(m_c
  -\overline m) = Q_{cc}/n_c^2 - 2Q_{ca}/(n_c n) + Q_{aa}/n^2$ with
  $Q_{cd}=\sum_v\mu^2(v)u_c(v)u_d(v)$ and $u_c(v)=\sum_{N\in c}
  \Lambda(N-v)/\sqrt{V(N)}$---the mask clears Bonferroni in every
  octave to $1.6\cdot10^7$, at $\max_c|z_c| = 8.4$ in the top band. At
  $N\approx10^8$ a $300$-versus-$300$ comparison gives the deep arm
  $0.9476\pm0.0293$ against the shallow $0.7238\pm0.0245$, a gap of
  $+0.2238\pm0.0056$. But its \emph{scaling} is not determined: over a
  factor $160$ in $N$ the data do not separate $N^{-a}$ from
  $(\log N)^{-b}$, and the fitted exponent varies about fourfold with
  the cell under either parameterisation, so no single exponent
  describes it. What matters here is unaffected: the mask is lower
  order and does not threaten $C(N)=o(N)$.
...
\subsection{How far the wall beats a coin}
...
Three quantitative checks, each with a bar fixed before the run. The
Moebius autocorrelation sits at $1.051$--$1.068$ times the random-sign
floor $\sqrt{0.32264\,(X-h)}$ --- not $\sqrt X$, since the sum sees
only $n$ with both $n$ and $n+h$ squarefree --- stably across five
decades of shift. The reconstruction gives $\rho-1=-0.0976$ against a
...
\paragraph{The rate, and the limit.}
Proposition~\ref{prop:W} with $c(h)\approx\mathfrak S_2(h)N$ and $S(h)$
at the square-root floor gives $\rho-1\asymp-b/\log N$. Fitting that
with the intercept fixed at $1$ --- a one-parameter model, and $1$ is
what Chowla forces --- costs $34\%$ in residual against a free
two-parameter fit, whose intercept sits at $1.039$, above $1$ and
walking down through $1.174, 1.088, 1.063, 1.070, 1.054, 1.039$ as the
window grows. The data have not reached the limit, so fixing it there
is premature at this size.

Both the fit and the rate it returned have since been withdrawn, and
the reason is worth stating because it is not a numerical one. The
quantity fitted is a \emph{de-trended} $\rho$, and de-trending across a
band of $N$ removes real variance: nearby $N$ share the same
$\mu(v)\Lambda(N-v)$ terms, so the $C(N)$ are positively correlated
across the band. Replacing $\mu$ by a random $\pm1$ on the same
support---leaving $\Lambda$, $V$ and every other input byte-identical
---reproduces the same estimator output. \textbf{The estimator cannot
tell $\mu$ from a coin}, so neither the drift nor the rate was a
measurement of $\mu$ at all. Proposition~\ref{prop:W} is an identity
and is untouched; what is gone is every number that claimed to confront
it. Whether $\rho\to1$---whether the wall is exactly square-root---is
therefore open, and with one realisation of $\mu$ it may not be
answerable at all.

\subsection{The margin, at the extreme}

The requirement constrains every $N$, so the figure to quote is the one
at the extreme, not at a typical $N$. With
$\max|G|\approx a_n$ the Gumbel location,
\[
  \max_{N\le X}|C(N)| \;\approx\; a_n\sqrt{\mathfrak A(N)\,N\log N},
  \qquad
  \frac{N}{\max|C|}\;\approx\;
  \frac{\sqrt N}{a_n\sqrt{\mathfrak A\log N}},
\]
which is $10^{4.4}$ at $N=10^{12}$ and $10^{22.8}$ at $N=10^{50}$. The
requirement is not remotely tight. The entire difficulty is in
\emph{proving} $o(N)$, not in the size --- and by
Theorems~\ref{thm:D} and~\ref{thm:Dprime} the one classical route to
proving it is closed over its whole design space.

\begin{remark}
Everything in this section is measured at $N\le1.6\cdot10^7$. The
no-go theorems of Section~\ref{sec:demand} are asymptotic and acquire
content only near $N\approx10^{480}$; nothing measured here constrains
that range, and nothing there is contradicted by these measurements.
\end{remark}
...
\subsection{C-III after the re-audit: what it needs, and what is left}
\label{sec:c3state}

Re-opening C-III raised three requirements. Two are now settled.
...
ones. So no change of variable carries one family to the other --- which
proves what an earlier probe had only measured, that the obvious
re-indexing is circular (the off-diagonal computed directly and via
that re-indexing agree exactly). What remains is the summation-formula
...
settled against the draft.} The draft's bookkeeping needs the
...
classification covers, flat in $J$ across the range the draft
proposes and increasing with $M$. Those pieces carry a rough
coefficient with no divisor structure and hence no Voronoi entry.
...
the parity obstruction. \textbf{Completing the draft and breaking the
wall are the same task.}
