...
and sixteen pre-registered closures}

\author{[author]}
\date{Working paper, increment 194 of the \texttt{goldbach-ln2}
program\\
...
that hypothesis from both of its sides and found both closed, for
sharply different reasons.
...
\textbf{sixteen pre-registered closures} --- five route adjudications
against the source papers' explicit lemma hypotheses, eight
kill-tested technique designs, and three representation-class
experiments --- each closed with numbers, together with the five
structural constraints they impose on any future technique.

The deliverable is the map, not a theorem toward Goldbach. We state
throughout what is and is not claimed, and we record twenty-eight
...
But & the weighted version is \emph{equivalent} to Goldbach
 (Theorem~\ref{thm:C}) & the field offers no coupling surface,
 internal or external (\S\ref{sec:closures})\\
\hline
\end{tabular}
\end{center}
...
\[
  ...
  \qquad
  \sum_{k\sim K}|D(k)|^2 \ll (\log N)^{-A}\sum_{k\sim K} M_k,
\]
for $K \le N^{1/3}$ dyadic. A fixed log-power saving is the currency;
$o(1)$ and $\log\log$ savings are not consumable.
...
\section{The negative map: sixteen closures}\label{sec:closures}
...
\subsection{Technique designs, kill-tested (8)}
...
C-III Motohashi type & (no finite test) &
 a full theory draft was written and then \textbf{refuted} by
 adversarial review at four independent coordinates: assembly scale
 mismatch $x^{2/3}$, a bookkeeping error hiding the type-II leaf, an
 illegitimate dual, and a pointwise budget deficit sealed by our own
 measurement.\\
\hline
\end{longtable}
