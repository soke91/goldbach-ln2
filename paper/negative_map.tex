...
one of which this paper withdraws}

\author{[author]}
\date{Working paper, increment 199 of the \texttt{goldbach-ln2}
program\\
...
C-III Motohashi type & (no finite test) &
 a full theory draft was written and then refuted by adversarial
 review at four coordinates --- \textbf{but see
 \S\ref{sec:reaudit}}: the first of them, the ``assembly scale
 mismatch $x^{2/3}$'', is \textbf{void}, having been measured against
 the misstated target. What stands is fatal to the draft as written (an
 illegitimate dual; a classification hiding a type-II region), not to
 the route.\\
\hline
\end{longtable}
...
\item \textbf{Four are in question}: the ``excess $K^{2/3}$'', the
  conductor-inflation half of the second-Vaughan-layer item, the
  L$^2$-versus-signed-L$^1$ mismatch (the same error class as the
  correction itself), and the arithmetic half of C-III's budget
  coordinate. Their budgets must be redone; they are neither
  reinstated nor void.
...
its remaining obstruction being a named one --- shift-averaged binary
$\mu\mu$ correlations at log-power strength.
