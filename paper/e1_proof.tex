...
\emph{Line-work remaining in this subsection:} the explicit
coprimality bookkeeping between $d, e, [k,k']$ and the $q$-layer
(finitely many gcd cases, each geometric), and the verification that
the type~I moduli excess $d[k,k'] > x^{1/2-\epsilon}$ mass is within
the boundary-band budget (an integral over the measured/Dickman
densities --- the same computation as T7's, shifted by $d$).
