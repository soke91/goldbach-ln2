...
DEPLOY = os.path.join(REPO, "deploy", "papers")
GEN = os.path.basename(ROOT)


def projection_map():
    """deploy/papers/<file> -> repo-relative source, from PROJECTS_FROM.

    배포본의 논문마다 어느 세대를 사영한 것인지 적어 둔 파일이다. 없으면
    빈 map 을 주고, 그러면 사영 검사들은 조용히 넘어간다 -- 없는 것을
    있다고 가정하는 것보다 안 하는 편이 낫다.
    """
    p = os.path.join(REPO, "deploy", "PROJECTS_FROM")
    out = {}
    if not os.path.exists(p):
        notes.append("G79/G81 deploy/PROJECTS_FROM absent; projection "
                     "checks skipped")
        return out
...
def g79_one_finding_both_trees():
    ...
    n = 0
    pairs = []
    for paper, src in sorted(projection_map().items()):
        tex = os.path.join(DEPLOY, paper)
        md = os.path.join(REPO, src.replace("/", os.sep))
        if os.path.exists(tex) and os.path.exists(md):
            pairs.append((paper, tex, src, md))
        else:
            notes.append(f"G79 {paper} or {src} absent; that pair skipped")
    for name, stale in PARITY:
        for paper, tex, src, md in pairs:
            hits = [w for w, p in (("deploy/papers/" + paper, tex),
                                   (src, md)) if stale in read(p)]
            if hits:
                fails.append(
                    f"G79 {' and '.join(hits)} still carries the signature "
                    f"of [{name}] -- a fix that reached one tree and not "
...
def _repo_evidence_index():
    ...
    idx = {}
    roots = []
    for gen in sorted(os.listdir(REPO)):
        g = os.path.join(REPO, gen)
        if not os.path.isdir(g) or not gen.startswith("v"):
            continue
        for sub in ("code", "results"):
            d = os.path.join(g, sub)
            if os.path.isdir(d):
                roots.append(d)
        ver = os.path.join(g, "verify")
        if os.path.isdir(ver):
            for pas in sorted(os.listdir(ver)):
                for sub in ("code", "results"):
                    d = os.path.join(ver, pas, sub)
                    if os.path.isdir(d):
                        roots.append(d)
    for r in roots:
