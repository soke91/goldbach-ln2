...
| 96 | *(#94 established the spectral component; significance is not size, and the law could not be stated without the size)* how much of the wall's variance is the zeros? | **0.39%, and that is a floor.** Regressing the de-masked, `√V`-standardised, band-standardised `Z(N)` on `cos(γ log N), sin(γ log N)` for the first ten ordinates gives `R² = 3.896·10⁻³` against a surrogate mean of `2.48·10⁻⁶` and a 100-surrogate **maximum** of `4.16·10⁻⁶` — **1571× chance**, established by the pre-registered rule. Every ordinate is individually significant: `z = 209.5, 95.1, 93.7, 51.9, 56.2, 57.1, 32.0, 36.4, 38.2, 23.4`. Amplitudes run `0.0644 → 0.0103` against the explicit formula's `2/\|ρ\| = 0.1414 → 0.0402`, i.e. the right shape at roughly a third the size. ⚠️ **0.39% is a FLOOR, not the share** — the zeros do not stop at the tenth, and quoting a truncated sum as a total is the same error class as #66. What it settles is that the component is **real and subdominant**, so Conjecture L's Gaussian half survives with a spectral term **named** rather than a new leading behaviour. No threshold was set on the share itself: the useful output is a number, and inventing a bar after seeing it would be hazard 2 |
