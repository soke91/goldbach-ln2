...
t = 0
while t < 300:
    k = int(rng.integers(300, 900))
    kp = int(rng.integers(300, 900))
    if k == kp:
        continue
    g = math.gcd(k, kp)
    L = k * kp // g
    ...
    for q in range(2, int(P1 ** 0.5) + 1):
        if sieve_p[q]:
            sieve_p[q * q:: q] = False
    ps = ps[sieve_p[P0:P1]]
    q_layer = 997          # q-스레딩 모델 (x^{1/2+δ} 축소판)
    a0 = int(rng.integers(1, q_layer))
    ps = ps[ps % q_layer == a0]
    w = N - ps * k
    wp = N - ps * kp
    ok = (w > 1) & (wp > 1)
...
np.savez("g4_qthread_ext.npz", rows=d)
