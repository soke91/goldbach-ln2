...
def balanced3(k, C):
    # 게이트-영역 판정 (근사): k = q·r·s, r²s ≤ k^{?}: 모델로
    # max(r,s) ≤ k^{1/2} and r*s ≥ k^{1/4} (q가 지배 인수 허용)
    fs = factors(k)
    if len(fs) < 3:
        return False
    lo, hi = k ** 0.25, k ** 0.5
    ...
    for assign in product(range(3), repeat=n):
        ...
        for f, a in zip(fs, assign):
            g[a] *= f
        q, r, s = sorted(g, reverse=True)
        if r <= hi and s <= hi and r * s >= lo:
            return True
    return False
