...
that we could reproduce is mask accounting. We report sixteen
pre-registered closures --- five route adjudications against the
source papers' explicit lemma hypotheses, eight kill-tested technique
designs, and three representation-class experiments --- together with
the four structural constraints they impose on any future technique. A
ninth design, K1, is left open: its orbit is not computable on the
object it addresses, and the measurement that closed it evaluated a
fifth of that orbit.
...
prime-pair-weighted $\mu$-correlation and record what a Chowla-type
input does and does not buy against it (Proposition~\ref{prop:W}),
...
\begin{remark}[what is classical here, and what is not]
\label{rem:bombieri}
The phenomenon is Bombieri's asymptotic sieve \cite{Bom76}, and no
claim of novelty is made for it. In Tao's exposition \cite{TaoBAS} the
governing fact is that \emph{the obstruction to the parity problem is
one-dimensional} --- a single unknown scalar $\delta_x\in[0,2]$ carries
all of it, with no bound on $\delta_x$ known even granting every other
major conjecture --- and that consequently ``one can replace any
parity-violating weight (such as $\Lambda$) with any other
parity-violating weight and obtain a logically equivalent estimate''.
Theorems~\ref{thm:D} and~\ref{thm:Dprime} are that statement
specialised to one route, and a reader who finds them unsurprising is
right to.

What that exposition does not contain, and what is offered here, is
the quantitative form: the loss is
$\exp(c\sqrt{(1-\theta_E)\log N})$, it arises as the separation
between two explicitly located thresholds --- $N^{\theta'}$, where a
weight's transform must carry mass to see $C(N)$, and
$N^{\theta_E-(1-\theta')}$, below which the residual is accessible ---
and it is stated as a function of the level $\theta_E$, so that the
$\theta_E\to1$ degeneration can be read off. Whether that form appears
elsewhere we have not established.
\end{remark}
...
Fix $X$ and let
\[
  C_X(N) \;=\; \sum_{\substack{n+v=N\\ n\le X,\ v\le X}}
                \Lambda(n)\,\mu(v)
\]
be the convolution truncated at $X$ in each argument, so that
$C_X(N) = C(N)$ for $N\le X$ and $C_X$ is supported in $N\le 2X$.
Then
\[
  \sum_{N\le 2X} C_X(N)^2 \;=\; \sum_{|h|<X} M(h)\,P(h),
  \qquad
...
\begin{proof}
Both sides count the same quadruples. Expanding $C_X(N)^2$ as a double
sum over $(n,v)$ and $(n',v')$ with $n+v = n'+v' = N$ and all four
variables in $[1,X]$, then summing over $N$, leaves the single
constraint $n-n' = v'-v =: h$ together with the box $n,n'\le X$,
$v,v'\le X$, which factorises into the $\Lambda$-pairs and the
$\mu$-pairs. This is Parseval for the two truncated sequences.
\end{proof}

\begin{remark}
The truncation carries the statement and is not cosmetic. Summing
$C(N)^2$ over $N\le X$ instead imposes $n+v\le X$, a simplex rather
than a box, and a sum over a simplex is not a product of two
one-variable sums: the box carries in addition every pair with
$X < n+v \le 2X$. That block is $37\%$ of the right-hand side at
$X = 4000$, and the ratio
$\sum_{N\le X}C(N)^2 \big/ \sum_{|h|<X}M(h)P(h)$ sits near $0.60$
across $X = 500$ to $16000$ with no tendency to $1$.
\end{remark}

The identity is exact and unconditional, and it makes the aggregate
size of the truncated wall a statement about two shifted correlations,
one of
...
\item \textbf{The tail is Gaussian too}, which is the half that
  ...
  octaves; aggregate tail counts against the Gaussian expectation are
  $21441$ against $21463$ at $t=3$ (ratio $0.999\pm0.007$) and $502$
  against $503.6$ at $t=4$ ($0.997\pm0.045$), while at $t=5$ there are
  $4$ exceedances against an expected $4.6$, whose Poisson error bar of
  $\pm0.43$ leaves the ratio uninformative in either direction; and the
  extremes are attained at generic $N$, not at deep radicals, so the
  mask removal is not leaking into the tail.

\item \textbf{The zeta ordinates are present in the field, and are not
  shown to be $\mu$'s.} Regressing $G$ on
  $\cos(\gamma\log N),\ \sin(\gamma\log N)$ for the first ten ordinates
  of the zeta zeros gives $R^2 = 4.6\cdot10^{-3}$. That figure has to
  be read against a null with the same serial structure as $G$: a
  permutation of $G$ is white, while the regressors sit in the lowest
  few dozen Fourier bins, which is where an autocorrelated field keeps
  its power. Against phase-randomised and circularly rotated
  surrogates the null mean is $1.8\cdot10^{-3}$, so the aggregate ratio
  is $1.4$. Against the control of Lemma~\ref{lem:coin} --- $\mu$
  replaced by signs on the same support, through the same $\Lambda$ and
  the identical pipeline --- the coin mean is $3.3\cdot10^{-3}$ and $4$
  of $20$ coin draws reach the measured value: \emph{the aggregate
  statistic does not distinguish $\mu$ from a coin.} A local
  comparison, judging each ordinate against control frequencies drawn
  from its own neighbourhood, does separate them --- $6$ of $10$
  ordinates clear their local $99$th percentile, against $0$ to $3$ for
  each of eight coins --- but on eight draws that is suggestive and not
  decisive. Since $C = \mu*\Lambda$ and $\Lambda$ carries the zeros by
  the explicit formula, the presence of the lines is not by itself
  evidence about $\mu$, and \textbf{whether any part of the spectrum is
  $\mu$'s rather than $\Lambda$'s is open}. None of this contradicts
  item~1: a small oscillatory component leaves kurtosis untouched, and
  \emph{Gaussian in distribution} and \emph{phase-random in $\log N$}
...
\begin{proposition}[The excess is a prime-pair-weighted
$\mu$-correlation]\label{prop:W}
With $u=N-p$ and $h=p'-p$, exactly,
\[
  \rho-1 \;=\; \frac1V\sum_{h\neq0}\ \sum_{p'-p=h}
    (\log p)(\log p')\,
    \bigl\langle \mu(u)\mu(u-h)\bigr\rangle_{\!p},
\]
the inner average running over the primes $p$ for which $p+h$ is also
prime. Replacing it by the unweighted average over all $u$ gives the
decoupled form
\[
  \rho-1 \;\approx\; \frac1V\sum_{h\neq0} c(h)\,S(h),
  \qquad
...
The first display is algebra: it is $C(N)^2 = V(N) + {}$off-diagonal
with the diagonal named. The second is not. It samples
$\mu(u)\mu(u-h)$ on the sparse set of $u$ for which $N-u$ and $N-u-h$
are both prime and reads off the average over all $u$, and that
substitution is unproven. The distinction is load-bearing, because
what the decoupled form is usually asked to deliver does not follow
from it.

$S(h)$ is the binary Chowla correlation, and it is an average, so
$|S(h)|\le1$. The coefficient multiplying it is not $O(1)$:
\[
  \Gamma(N) \;:=\; \frac{\sum_{h\neq0}c(h)}{V(N)}
   \;=\; \frac{\theta(N)^2-\sum_{p<N}(\log p)^2}{V(N)}
   \;\sim\; \frac{N}{\AAA(N)\log N},
\]
measured at $1.55\cdot10^{3}$, $1.85\cdot10^{4}$, $3.58\cdot10^{5}$ at
$N = 10^4$, $1.6\cdot10^5$, $4\cdot10^6$, with $\Gamma\log N/N$
approaching $1/\AAA = 1.270$. Since $c(h)\ge0$, a hypothesis
$|S(h)|\le\varepsilon$ yields nothing better than
$|\rho-1| \le \varepsilon\,\Gamma(N)$. \textbf{So $S(h)=o(1)$ --- what
Chowla's conjecture asserts --- falls short of what is needed by a
factor $N/\log N$}; the strength required is $S(h) = o(\log N/N)$.

Nor does any bound on $|S(h)|$ suffice. Measured directly, the
absolute budget $\sum_{h}c(h)|S(h)|/V$ is $13.5$, $17.5$, $23.3$,
$30.7$ at $X = 2\cdot10^4,\dots,1.6\cdot10^5$ --- above $1$ by an
order of magnitude, and growing. Whatever sends $\rho$ to $1$ must be
\emph{signed cancellation across $h$}, a different object from the
smallness Chowla asserts and from the averaged absolute bound of
\cite{MRT15}. Whether $\rho\to1$ --- whether the wall is exactly
square-root --- is open.
...
squarefree --- stably across five decades of shift. The decoupled form
of Proposition~\ref{prop:W} evaluates, at $X = 4\cdot10^6$, to
$-0.124$ when $S(h)$ is normalised as $M(h)/X$ and to $-0.052$ when it
is taken as the average it is defined to be, $M(h)/(X-h)$; at
$X = 10^6$ the same two conventions give $-0.101$ and $-0.015$. The
sign is negative throughout, and that is the stable part of it. No
comparison against a \emph{measured} $\rho-1$ is drawn: by
Lemma~\ref{lem:coin} the centred estimator of $\rho$ returns the same
value on $\mu$ as on a coin --- measured, the real $\rho$ sits within
$0.94$ standard errors of the coin's in every octave --- so a measured
$\rho-1$ is not a measurement of $\mu$ and cannot calibrate a
...
These are for $S(h)=M(h)/X$; taking $S(h)=M(h)/(X-h)$ moves the top bin
to $41.6\%$ and the cancellation factor to $6.6$, and leaves the
...
output on $\varepsilon$ as on $\mu$. The exact display of
Proposition~\ref{prop:W} is algebra and is untouched; its decoupled
form is not, no fitted rate for $\rho-1$ is claimed here, and no
measured $\rho-1$ is used to calibrate a reconstruction of one.
\textbf{Whether $\rho\to1$ --- whether the
wall is exactly
...
$n_c^{-1/2}$. Collecting the lemma's three terms into one square,
\[
  \mathrm{Var}\bigl(m_c-\overline m\bigr)
  \;=\; \sum_v \mu^2(v)
        \Bigl(\frac{u_c(v)}{n_c}-\frac{u_a(v)}{n}\Bigr)^{\!2},
\]
each bracket is a difference of two cell averages of
$\Lambda(N-v)/\sqrt{V(N)}$. For fixed $v$, about $n_c/\log N$ of the
terms of $u_c(v)$ are nonzero, each of size $\log N/\sqrt V$, and they
all carry the same sign, so $u_c(v)/n_c \approx
\mathfrak s_c(v)/\sqrt V$ with $\mathfrak s_c$ the local density of
$N-v$ over primes in the cell. Hence
\[
  \mathrm{Var}\bigl(m_c-\overline m\bigr)
  \;\approx\; \frac{\sum_v\mu^2(v)}{V}\,
     \bigl\langle(\mathfrak s_c-\mathfrak s_a)^2\bigr\rangle
...
The cell factor $\langle(\mathfrak s_c-\mathfrak s_a)^2\rangle$ cannot
be dropped, and this is why the proposition is stated on the square of
the difference rather than on $Q_{cc}/n_c^2$: the first term of
Lemma~\ref{lem:cellmom} exceeds the variance by a factor between $2$
and $60$ depending on the cell, the other two cancelling it to exactly
that order. What the three terms share is the factor
$(\sum_v\mu^2)/V$, and that is what carries the $N$-dependence the
display measures. Computed exactly, $\mathrm{Var}\cdot\log N$ is
constant to $1\%$ across a factor $4$ in $N$ at every depth, against
the factor $4$ an $n_c^{-1/2}$ law would produce.
...
MRT / Lichtman 2020, shift $\to$ dilate & Blocked &
 ...
 family. \emph{Checked against the source}: \cite{Li20}'s Lemma~2.1
 turns the pair constraint into
 $\mathbf 1_{m-n=m'-n'}=\int_0^1 e((m-n-m'+n')\alpha)\,d\alpha$, an
 identity available only because the constraint is a difference.\\
Tao 2016 entropy decrement, $k$-averaged & Blocked $\times 3$ &
 ...
 triple-log, below spec. \emph{Checked against the source}
 \cite{Tao}: (i) is that paper's own stated obstruction --- ``the
 logarithmic averaging is unfortunately needed in our method in order
 to obtain an approximate affine invariance in the $n$ variable'';
 (ii) is its mechanism, $\lambda(pn)=-\lambda(n)$ at small primes
 $p$; and (iii) is its Remark~1.4, which gives the decay as
 $O(\log x/(\log\log\log x)^c)$.\\
...
K1 & multiplicative Fej\'er kernel on the exact dilation ladder &
 \textbf{open}: the orbit is not reachable on the object it is about.
 On the type-II field the column at $s$ is empty unless
 $sk<\sqrt N$, so at $N\approx2\cdot10^8$ with $k\in[500,900]$ only
 $13$ of the $63$ divisors are live ($17.3\%$ of the design matrix)
 and the measured $R^2 = 0.4664$ is that of a $13$-column design. A
 live orbit needs $N > (30030\,k)^2$, which is $3.6\cdot10^9$ already
 at $k=2$. On the \emph{untruncated} field, where all $63$ columns are
 live, the same statistic reads $0.904$ against a permutation null of
 $0.165\pm0.051$ and a coin null of $0.44$ --- inside the band the
 pre-registration reserved for ``repeat at a second $N$ before
 deciding''. \S\ref{sec:R4} names the mechanism: the type-II cut makes
 every divisor sum incomplete.\\
...
R1 & zero-spectrum visibility (explicit formula) &
 ...
 $0.2196\pm0.0055$, the measurement $-0.80$ standard errors below its
 null. Two cautions on the precision. The design puts $60$ regressors
 on $300$ points, so $p/n = 0.2000$ of both figures is free
 parameters; that floor cancels from the comparison, since both
 template sets carry it, but not from a percentage. Three standard
 errors is $7.5\%$ of $0.2196$ and $84\%$ of the genuine component
 $0.2196-0.2000$, so what is excluded is the zeros contributing about
 twice what random frequencies do, and nothing finer. And the
 $\pm0.0055$ is a six-draw spread: rebuilt at $40$ draws, a six-draw
 standard deviation runs about half the forty-draw one.\\
R2 & determinant / Kloosterman phase &
 \textbf{dead on the measurement, not on the criterion, and not
 blind}. The regression arm: $R^2 = -0.00012$ against a random-phase
 control of $+0.00002 \pm 0.00030$, i.e.\ $z = -0.46$. The
 coherent-gain arm: the phase $e(-N\bar{k'}/k)$ gives $G_1 = 1.513$
 against a random-phase null of $0.994 \pm 0.187$, i.e.\
 $z = +2.78$; the mirror phase gives $z = +1.02$. The design's bar
 $G \ge 2$ was set as an effect size against a theoretical null of
 $1$, and sits at $z = +5.38$ of the null the design actually has,
 so the arm is a five-and-a-half-sigma test. The pairs entering the
 regression number $11{,}572$, from a design of $40\times600$.\\
...
$B=512$, its precision degrading as $\sqrt{2/n_b}$; the C4
threshold of $0.5\times$ sits only $1.29$ standard errors below its
null, so noise satisfies it $8.8\%$ of the time; and R2's
coherent-gain bar $G\ge2$ sits $5.38$ standard errors above its null,
so that arm could not have fired on anything short of a very large
effect. In each case the DEAD direction stands on the measurement ---
never systematically below the null --- and not on the count of
flagged levels.

Two of these designs carry a criterion that their reported numbers do
not decide, and we say so rather than leave it implicit. R2's
regression arm is written as ``capture $\ge 2\times$ the random
control'', and the control's own mean is zero to within its spread
($+0.00002 \pm 0.00030$), so ``twice the control'' is not a bar in
either direction: at the run reported as $R^2 = +0.0004$ it would have
been cleared. R2 is DEAD because $z = -0.46$, and the criterion it was
written against decides nothing. The same reading applies to any
threshold of the form ``$\ge c\times$ control'' when the control is
...
\begin{definition}[the block ratio, and its null]\label{def:blockratio}
Let $\mathrm{supp}(k)$ be the number of $m$ in the type-II range with
$\mu(m)\mu(N-mk)\neq0$, so that $\mathrm{supp}(k)$ is the exact second
moment of $D(k)$ under random signs. Partition the $k$-band into
consecutive blocks of $B$. Two statistics are in use and they are not
the same:
\[
  r_{\mathrm{supp}}(B)^2 =
    \frac{\sum_j S_B(j)^2}
         {\sum_j\sum_{k\in\text{block }j}\mathrm{supp}(k)},
  \qquad
  \rho(B)^2 =
    \frac{\sum_j S_B(j)^2}{\sum_k D(k)^2}.
\]
Both equal $1$ in expectation under the independence
Conjecture~\ref{conj:L} predicts. The first also tests
$|D(k)|\asymp\sqrt{\mathrm{supp}(k)}$; the second holds the magnitudes
fixed and tests \emph{only} the coupling across $k$, which is what the
switch would leave behind. The null for $\rho$ is therefore
$D(k)\mapsto\varepsilon_k D(k)$ with $\varepsilon_k=\pm1$
independent: it preserves every $|D(k)|$ and destroys nothing else.
\end{definition}

\begin{remark}[why the null is not optional here]
Neither statistic can be read against the value $1$. Since
$\mathrm{supp}(k)\asymp N/k$, the five smallest $k$ carry about $38\%$
of $\sum_k\mathrm{supp}(k)$, so the effective number of independent
blocks is a handful whatever $B$ is, and the sampling spread is
enormous. Measured at $N=1.6\cdot10^7$,
$r_{\mathrm{supp}}(B) = 1.75, 1.18, 0.91, 0.78, 0.68$ at
$B=1,2,4,8,16$ --- a fall by a factor $2.6$, which is exactly the
signature this test was built to detect, and which would read as a
surviving residue. It is not one. Against the sign-randomisation null
of Definition~\ref{def:blockratio}, $\rho(B)$ sits at
$z = -1.03, -1.15, -1.19, -1.27, -1.19, -0.86$ for
$B = 2,\dots,64$, with null standard deviations of $0.28$ to $0.46$;
at $N = 4\cdot10^6$ the same numbers are $z = -1.57$ to $-0.16$.
Nothing reaches two standard errors at any block size. \textbf{The
block-ratio diagnostic carries almost no power}, and the flat reading
and the falling reading are both inside its noise. What settles R4 is
the lag-1 statistic below, whose null is the same and whose spread is
$n^{-1/2}$.
\end{remark}

\emph{Result: none of it survives.} On $8000$ values of $k$ --- the
entire band on which the type-II field is non-empty, since
$m > \sqrt N$ forces $k < \sqrt N$ --- no block size puts $\rho(B)$
below its null by as much as two standard errors, and the diagnostic
that does carry power, the lag-1 autocorrelation of
$D(k)/\sqrt{\mathrm{supp}(k)}$, reads $+0.0104$ and $+0.0127$ against
a standard error of $0.0112$ at $N\approx6\cdot10^7$, and $-0.0446$
and $+0.0173$ against $0.0264$ and $0.0186$ at $N=4\cdot10^6$ and
$1.6\cdot10^7$: dead zero at every scale, and nowhere the negative
coherence a surviving residue would require. That standard error is
$n^{-1/2}$, the value under the independence being tested; a block
bootstrap, which assumes none, returns the same figure to two
decimals.
...
M\"obius-side $a$ to satisfy $a \le y^{O(1)} = M^{o(1)}$.

\begin{definition}[the absolute Heath--Brown weight]\label{def:hbw}
Heath--Brown's identity of level $J$ with cut $z = M^{1/J}$ splits each
$n\le M$, in its $j$-th term, into a M\"obius side $a = m_1\cdots m_j$
with every $m_i\le z$ and a complementary side $b = n_1\cdots n_j$.
Taking absolute values throughout, put
\[
  A_j(a) = \#\{(m_1,\dots,m_j):\ \textstyle\prod_i m_i = a,\
     m_i\le z,\ \mu(m_i)\neq0\},
  \qquad
  D_j(y) = \sum_{b\le y}\tau_j(b),
\]
and
\[
  W(a) \;=\; \sum_{j=1}^{J}\binom{J}{j}A_j(a)\,D_j(M/a),
\]
so that $W(a)$ is the number of terms of the identity whose M\"obius
side is exactly $a$, and $\sum_a W(a)$ is its total term count. The
quantity reported below is $\sum_{a>M^{0.05}}W(a)\big/\sum_a W(a)$.
\end{definition}

Measured, that fraction is $0.933, 0.942, 0.958, 0.961$ at
$J = 3, 4, 6, 8$ and $M = 10^5$, rising to $0.952, 0.961, 0.970,
0.977$ at $M = 10^6$. So about $95\%$ of the identity's weight lies
outside the region the classification covers, roughly flat in $J$
across the admissible range and increasing with $M$. Those pieces
carry a rough coefficient with no divisor structure and hence no
Voronoi entry.

\begin{remark}[what precision this table supports]
Two decimals, not three. The rounding convention for $z$ alone ---
$\lceil M^{1/J}\rceil$ against $\lfloor M^{1/J}\rfloor$ --- moves the
$J=8$, $M=10^5$ entry from $0.961$ to $0.944$, and the $J=6$ entry by
$0.009$; at $J = 3, 4$ it moves nothing, because $z$ is large enough
there that one unit does not matter. The conclusion is a statement
about $95\%$ against $5\%$ and is untouched by any of this, but a
third decimal is a convention and not a measurement.
\end{remark}
...
concentrates in exactly those high-$j$ terms: in the weight of
Definition~\ref{def:hbw} at $M=10^5$, $0.81$ sits at $j=3$ when
$J=3$, and $0.84$ at $j \in \{6,7,8\}$ when $J=8$, against $0.003$ at
$j\le3$. A repair would have
...
a cost of at most a log power must satisfy all four of the following.
A fifth, (K1), is stated and withdrawn.

\begin{enumerate}
\item \textbf{(K1)} \emph{Not established.} The intended constraint
  was that $T$ cannot act through divisibility sub-sums, on the
  ground that the multiplicative orbit spans only half the field's
  energy. The measurement behind it evaluated $13$ of the $63$
  divisors, and on the full orbit of the untruncated field the orbit
  spans $0.90$, so this row constrains nothing until the orbit can be
  computed on the type-II field. The other four constraints do not
  depend on it.
...
progressions, determinant phases carrying at most a $+2.8$-sigma
coherent gain and nothing in regression --- the field offers no
coupling surface in any direction that has an existing mathematical
name, at the strength each test can see. In the automorphic world, shifted convolutions
...
\item The supply side is closed to measurement: the object is
  featureless in every direction with a name, and the sixteen
  pre-registered closures say precisely which directions were checked
  and why each fails. One design, K1, is not closed.
...
conjecture}, in \emph{Combinatorial and Additive Number Theory IV}
(CANT 2020), Springer Proc. Math. Stat.\ \textbf{347}, 2021;
arXiv:2005.03811v2 [math.NT], 2022. Equation numbers here are those of
the arXiv version.
\bibitem{Bom76} Enrico Bombieri, \emph{The asymptotic sieve},
Rend. Accad. Naz. XL (5) \textbf{1/2} (1975/76), 243--269.
\bibitem{TaoBAS} Terence Tao, \emph{Notes on the Bombieri asymptotic
sieve}, blog post, 17 July 2016.
