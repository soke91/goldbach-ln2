...
\]
At the top the measured value is a factor $434$ above the worst of
those twenty draws. That factor is not a number of standard
deviations: no standard deviation of the sign ensemble is computed
anywhere here, and the worst of twenty draws from a half-normal is
about $2\sigma$, so the separation in units of $\sigma$ is larger than
$434$ rather than smaller. The factor is quoted because it is what was
measured.
...
quote is the one at the extreme rather than at a typical $N$. At the
extreme a size has to be borrowed rather than measured, and the loan
is worth naming. \emph{Suppose} $C(N)=\sqrt{V(N)}\,G(N)$ with $G$
behaving across $N$ like independent standard Gaussians. That is
precisely the distributional statement \S\ref{sec:notclaimed} declines
to assert; it is admitted here, in this section only, and nothing in
this paper rests on what follows. Then
...
with $a_n=\sqrt{2\log(N/2)}$ and $\AAA=0.787275$. That is the value of
\eqref{eq:constants} on the family the sweep uses --- $N$ whose only
odd prime factor is $5$ --- and not a generic one: $\AAA$ depends on
the prime support of $N$, and an even $N$ not divisible by $5$ carries
the extra local factor $1-\frac{1}{5\cdot4}$, so it gives
$0.95$ times as much
(\texttt{audit\_margin.py}, \texttt{audit\_constants.py}). Those two
$N$ lie five and thirty-eight orders of magnitude beyond anything
measured here, and the digits are the arithmetic of the display under
the supposition, not a measurement. Under it the quantity is
expected to be small by a wide margin and is provably
...
\texttt{audit\_cn\_coin\_deep.py} & \texttt{audit\_cn\_coin\_deep.txt}
 & the $0.6455$ of \S\ref{sec:controls}\\
\texttt{audit\_margin.py} & \texttt{audit\_margin.txt}
 & the extrapolation of \S\ref{sec:margin}\\
\texttt{audit\_constants.py} & \texttt{audit\_constants.txt}
 & $\AAA=0.787275$, \S\ref{sec:margin}\\
\hline
\end{tabular}
\end{center}
