...
record a defect in \cite{HL}, first observed by S.~Zheleznov:
equation~(18) drops an $n$-dependent constraint present in the
definition of $S_2(\alpha)$. The missing term $\Delta$ is exhibited and
shown harmless under the hypotheses already assumed
...
The published form of equation~(18) drops a range. What follows records
the defect and one way of closing it. Neither is claimed as new: the
observation is S.~Zheleznov's, and the authors have since corrected the
manuscript by a different route, on which the dropped range never
appears. What is set down here is kept because the mechanism it uses is
the one Theorem~\ref{thm:A} rests on, and because
Proposition~\ref{prop:movingcut} below shows the two routes meet.
