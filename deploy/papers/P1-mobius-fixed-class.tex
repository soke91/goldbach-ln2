...
record a defect in \cite{HL} --- reported to the authors
independently, and before this note, by S.~Zheleznov: equation~(18)
drops an $n$-dependent constraint present in the definition of
$S_2(\alpha)$. The missing term $\Delta$ is exhibited here and shown
harmless under the hypotheses already assumed
(Section~\ref{sec:delta}). The authors have since corrected the
manuscript by keeping the constraint instead, so that $\Delta$ does not
arise there; the two formulations differ by exactly $\Delta$'s shape,
and Theorem~\ref{thm:A} together with that estimate gives the bound
against the corrected one (Proposition~\ref{prop:movingcut}).
...
Pan \cite{Pan82}, and \cite{Vat19} develops the equidistribution side.
...
summation and \cite{HL} spend it on $E_4$ through their Lemma~4, and
...
\begin{equation}\label{eq:Bsum}
  B(N)\;:=\;\sum_{\substack{k<K\\ (k,N)=1}}
     \mu^2(k)\,(\log k)\,\bigl|\Emu(N;k)\bigr|,
\end{equation}
the restriction to squarefree $k$ being what the triangle inequality
on \eqref{eq:E3} leaves, since $\mu(k)=0$ elsewhere. This is sharper than the quantity
\cite{HL} actually reach before appealing to $EH_\mu$, namely
...
cosmetic. Assigning a main term $\Tcut_m/\varphi(q)$ to a degenerate class
...
for the second use Bombieri--Vinogradov \cite{MoV} with exponent
$2A+C_1(B)$.
\end{proof}
...
+O(M\log N/D_0)$ with $\Tcut_m(t)$ as in \eqref{eq:Tm}. Put
...
have odd radical $5$ and \emph{one} threshold: the sweep moves the size
of $N$ by a factor $128$ and its arithmetic not at all.
...
three of the seven and fails at four. The mechanism is visible: when
$N$ carries several small odd primes, $k$ and $m$ must both avoid them,
so the surviving $m$ are almost all prime and $\mu(m)=-1$ dominates. At
$N=2\cdot3^2\cdot5\cdot7\cdot11\cdot13\cdot17$ all $513$ terms of $E_3$
are negative and $|E_3|=B(N)$ exactly --- no cancellation across $k$ at
all, against $442$ of $562$ at the swept $N$.
...
follow and they carry different credit. \textbf{The defect is not
claimed as new}: that the $n$-dependent restriction had been replaced
by a fixed outer range was reported to the authors by S.~Zheleznov
before this note was written, and they have since prepared a corrected
manuscript. \textbf{The missing term and its treatment are what this
section sets down}, and nothing on record attributes them elsewhere.
The two are different kinds of work and neither replaces the other. A
correction that keeps the moving truncation repairs the derivation
\emph{from the inside}: the range $m\le\alpha$ never appears, so there
is no term to bound. What follows is the \emph{outside} version: the
published form is taken as it stands, the discrepancy is named, and it
is bounded. The second is not the first done again --- the term
\emph{is} the difference between the two formulations, and that is what
lets a theorem proved against the fixed cut transfer to the corrected
one (Proposition~\ref{prop:movingcut}).
...
$b\le B:=(\log N)^{A+4}$ --- the same one Huang--Li use in their \S3.1
...
which lies in the range of its supremum; this is the only place
$\theta'>1/2$ is used.
...
$\log m$ replaced by $1$ and close by the argument above verbatim, the
main term dying by Lemma~\ref{lem:mu} applied directly --- the weight
being $1$, no partial summation is needed to remove a $\log m$.

For $\Xi_{\mathrm{mean}}$, since $n>Y_k$ iff
$k>R_n:=\lceil(N-n)/\alpha\rceil-1$, exchanging gives
$\Xi_{\mathrm{mean}}=\sum_{n<N}a_n(\rho_N(K)-\rho_N(R_n+1))$ with
$\rho_N$ as in \cite[Lem.~14]{P2} and
$a_n=\Lambda(n)\mu(N-n)$. Split at $N-n=H$ with
...
threshold the no-go of \cite{P2} turns on, and not by accident: below
it the cofactor left by the completion is no longer short.
\end{note}
...
each agree to $2.274\cdot10^{-18}$ and $4.547\cdot10^{-18}$ relative to
$N$, accumulated in index orders that share nothing, and the largest
...
(Bombieri--Vinogradov \cite{MoV}; the Goldston--Y{\i}ld{\i}r{\i}m
...
work, \cite{LiWF}; we cite it here as the object the question below
...
\cite{HL}, unconditionally for $\theta'>1/2$. The two formulations
differ by a term of $\Delta$'s shape, so the fixed cut is not
...
\bibitem{LiWF} J.~D.~Lichtman, \emph{Primes in arithmetic
...
\bibitem{Pan82} C.-D.~Pan, \emph{A new attempt on Goldbach conjecture},
Chinese Ann. Math. \textbf{3} (1982), 555--560.

\bibitem{MoV} H.~Montgomery and R.~Vaughan, \emph{Multiplicative number
