...
power of $\log N$, and which destroys the extraction \emph{even when
the hard input is granted for free} --- the switch identity carries a
third term $T_w$, of size $N$, and the theorem hands over the bound on
it that only $EH_\mu$ could supply;
Theorem~\ref{thm:Dprime} shows the separation
...
inner factor $\log(N-n)$ separating the two \cite[Cor.~2]{P1}. $E_4$ is
...
The term $T_w$ in \eqref{eq:extract} is \eqref{eq:Tw} itself, and it is
of size $N$: at $w=\log$ it \emph{is} $E_3(\alpha)$, which
\cite[Thm.~3]{P1} evaluates as
$\tilde r(N)-\SS(N)(N-C(N))+O_A(N(\log N)^{-A})$. It cannot be absorbed
into the $O(N^{o(1)})$, and writing \eqref{eq:extract} without it turns
...
$|T_{\log}|/N$ at those $N$ (Measurement~\ref{meas:flatsum}). With
$T_w$ restored the identity holds to $10^{-16}$ relative.
...
weight for which one is known is $w=1$, by \cite[Thm.~1]{P1}. The
...
itself --- at $\theta'=0.56$ they add $5.4\cdot10^{-2}N$ at $w=1$ and
$2.9\cdot10^{-1}N$ at $w=\log$, the latter being two thirds of
$|T_{\log}|$.
\end{note}
...
$N^{-0.3505}$ and $|T_{\log}|/N$ as $N^{-0.2658}$: the gap widens, as
it must if one side is $\ll_AN(\log N)^{-A}$ and the other is
$\asymp N$. The index set here runs from $k=1$, as \eqref{eq:Tw} and
\eqref{eq:flatsum} require; note that $H(N;1)=C(N)$ exactly, so
dropping that one term removes $C(N)$ from the flat sum --- which
changes the ratios above by up to $10.4\%$ and the fitted exponent to
$-0.3620$, while leaving $|T_1|$ untouched to $10^{-18}$, since the
...
Being an ordinary Bombieri--Vinogradov object is a statement about
shape, not about level, and the level is the whole question. Verified
...
the least $Q$ past which the deficit stays under $0.01N$,
\[
  Q^*=160112,\ 200848,\ 482589,\ 589203,
  \qquad
  Q^*/\sqrt N=358.0213,\ 317.5686,\ 539.5509,\ 465.8059,
\]
so $Q^*$ exceeds $\sqrt N$ by two orders at every $N$ tested.
Expanding the squarefree condition buys the ordinary modulus and costs
one to two orders in its size --- which is the barrier again, in the
one place the identity had made it measurable
(\texttt{lab\_combined\_modulus.py}).
\end{measurement}

\begin{proposition}[the wall cancels out of the count]%
\label{prop:direct}
Substituting \eqref{eq:posweights} into the identity of \cite[Thm.~3]{P1},
...
constant-factor condition of \cite[Prop.~6]{P1}: a bound at level
...
comparison. (For the numerator of \cite[eq.~(9)]{P1}, which keeps the
...
the parenthetical list, not the two above it).
\end{measurement}
...
Measurement~\ref{meas:extract} prints $|B_w|\varphi(d_0)=1.000000$, no
...
\begin{proof}
The residual of the switch is \cite[eq.~(16)]{P1}, which after unfolding
...
$A>0$ --- which is what \cite[Thm.~1]{P1} supplies unconditionally at
$w=1$, and the most that $EH_\mu$ would supply for a general $w$ ---
identity \eqref{eq:extract} yields at best
\[
  |C(N)|\;\ll_A\;\exp\!\left(c_1\sqrt{\tfrac12\log N}\right)
   \cdot\frac{N}{(\log N)^{A}},
\]
which is not a saving of any power of $\log N$. No weight in the
design space extracts $C(N)$ by divisor switching, not even when the
hard input is given for free.
\end{theorem}
...
For the second assertion, solve \eqref{eq:extract} for $C(N)$ and bound
the three terms on the right. The complete part is
$\ll\|b\|_1\log N$ by Lemma~\ref{lem:Gb} and $\Lambda\ll\log N$; the
residual is $\ll_A\|b\|_1N(\log N)^{-A}$ by Lemma~\ref{lem:bv}, the
support hypothesis on $b$ being exactly what that lemma needs; and
$T_w\ll_AN(\log N)^{-A}$ is the granted hypothesis. Since
$\|b\|_1\ge|b_1|$ and the first two dominate,
\[
  |C(N)|\;\le\;\frac{\|b\|_1}{|B_w|}
     \cdot O_A\!\left(\frac{N}{(\log N)^{A}}\right)
  \;\ll_A\;\exp\!\left(c_1\sqrt{\tfrac12\log N}\right)
     \frac{N}{(\log N)^{A}},
\]
by \eqref{eq:loss}. Finally $e^{c_1\sqrt{\log N/2}}$ exceeds every
power of $\log N$, so the saving is destroyed for every $A$.
\end{proof}
...
identity of \cite[Thm.~3]{P1} then looks like an accident of one
...
cancellation is available and $\mathrm{CP}_D\ge0$. The upper bound is
the classical sieve upper bound for prime-plus-almost-prime sums,
applied to each $r$-piece. No lower bound of the same order is claimed:
...
% =====================================================================
\section{The circle method has zero margin on
...
Bombieri--Vinogradov-accessible extracts $C(N)$, and this holds even
when the bound on the switch identity's third term $T_w$ --- the object
the whole problem is about --- is granted for free. The loss is
